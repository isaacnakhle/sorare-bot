...
MAX_FLOOR_FOR_ALERT = 5.0    # only alert if the floor itself was < $5
HARD_PRICE_CAP = 3.0         # extra safety net: never alert above this,
...
CONCURRENCY = 10              # parallel requests within a batch
...
# NOTE: best-guess field name for bulk player enumeration - verify/adjust
# against https://api.sorare.com/graphql (introspection) if this errors.
ALL_PLAYERS_QUERY = """
query AllPlayers($cursor: String) {
  football {
    allPlayers(first: 100, after: $cursor) {
      nodes {
        slug
      }
      pageInfo {
    ...
    while True:
        try:
            resp = requests.post(
                SORARE_API_URL,
                headers=HEADERS,
                json={"query": ALL_PLAYERS_QUERY, "variables": {"cursor": cursor}},
                timeout=20,
            )
            ...
            if "errors" in data:
                print(f"Player list GraphQL error (page {page}): {data['errors']}")
                break

            block = data["data"]["football"]["allPlayers"]
            ...
            print(f"Player list fetch failed on page {page}: {e}")
            break
...
def get_all_player_slugs():
    ...
    print("Fetching full player list from Sorare...")
    ...
    try:
        ...
        for card in player["cards"]["nodes"]:
            ...
            if offer and offer.get("price") is not None:
                try:
                    listings.append((float(offer["price"]), card["slug"]))
...
def main():
    ...
    if not all_slugs:
        print("Could not get a player list - check ALL_PLAYERS_QUERY / API token.")
        return
