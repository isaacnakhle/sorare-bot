...
DISCOUNT_PERCENT = 0.10      # alert if new listing is >= 10% below floor
...
def alert(player_slug: str, card_slug: str, display_price: float, previous_floor: float, season_type: str):
    ...
    text = (
        ...
        f"Discount: ${discount:.2f} ({discount_pct:.0f}%) below floor\n\n"
...
def process_results(results, floors: dict):
    for slug, by_season in results:
        for season_type, listings in by_season.items():
            ...
            # Only ever lower the tracked floor, never raise it - this makes
            # it a true rolling minimum instead of "whatever price I last
            # happened to see" (which was letting real drops slip through
            # when the price drifted down gradually instead of in one jump).
